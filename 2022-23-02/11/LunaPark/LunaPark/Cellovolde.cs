    ...
    class Céllövölde
    {
        ...
        public void Kiállít(Ajándék ajándék)
        {
            if (ajándék.Céllövölde != null)
                throw new FoglaltAjándékException();
            if (Ajándékok.Contains(ajándék))
                throw new LétezőAjándékException();
            ajándék.Céllövölde = this;
            Ajándékok.Add(ajándék);
        }

        public void Regisztrál(Vendég vendég)
        {
            Vendégek.Add(vendég);
        }
        ...
        {
            if (Vendégek.Count == 0)
                throw new NincsVendégException();
