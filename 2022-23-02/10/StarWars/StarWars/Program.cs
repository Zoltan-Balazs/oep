        ...
        static void Main()
        {
            ...
            try 
            {
                ...
                for (int i = 0; i < n; ++i)
                {
                    ...
                    if (null == planet) throw new NotExistingPlanetException();
                    ...
                    for (int j = 0; j < m; ++j)
                    {
                        ...
                        switch (type)
                        {
                            case "Breaker": ship = new Breaker(shipname, s, a, g); break;
                            case "Lander":  ship = new Lander(shipname, s, a, g); break;
                            case "Laser":   ship = new Laser(shipname, s, a, g); break;
                        }
                        ship.Protect(planet);
                    }
                }

                if (ss.MaxFireShip(out StarShip starship)) Console.WriteLine($"Best firepower starship: {starship.name}");
                else Console.WriteLine("There is no planet with starships");
                
                Console.WriteLine($"shield of Earth: {ss.SearchPlanetbyName("Earth").TotalShield()}");
