...
namespace ATM
{
    class Bank
    {
        ...
        {
            if (null == account) throw new NonExistingAccountException();
            if (!customers.Contains(customer)) throw new IllegalTransaction();
        ...
        public Account AccountSearch(string cardNo)
        {
            ...
            {
                if (acc.CardSearch(cardNo)) return acc;
            }
            return null;
        }
    }
